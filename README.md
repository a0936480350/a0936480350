# 👋 Hi, I'm Mike

物流系統開發工程師 → 持續轉型全端開發  
主要使用 C# / ASP.NET Core / SQL，擅長把需求變成可上線的產品。

![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET_8-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![ASP.NET Core](https://img.shields.io/badge/ASP.NET_Core-5C2D91?style=flat-square&logo=.net&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=flat-square&logo=microsoft-sql-server&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)

---

## 📚 作品集目錄

### 🎯 [DevLearn — 程式設計學習平台](https://github.com/a0936480350/DevLearn) ⭐ 主力作品

> 一個完整的線上學習平台。232 章教學內容、8 款遊戲化學習、老師媒合、即時聊天、AI 輔助開發。

| 面向 | 內容 |
|------|------|
| 🌐 **Live Demo** | [devlearn-dotnet.azurewebsites.net](https://devlearn-dotnet.azurewebsites.net/) |
| 🛠️ **技術棧** | ASP.NET Core 8 MVC · EF Core · PostgreSQL · SignalR · Phaser.js 3 |
| 📊 **規模** | 60+ DB tables · 30+ Controllers · 232 章節 · 500+ 題目 |
| ✨ **特色** | AI 輔助開發、遊戲化學習、即時多人互動、完整的後台管理系統 |

**技術亮點：**
- 🎮 **Phaser.js 2D 遊戲引擎**：實作程式碼大富翁（vs AI 對戰、多人本地對戰）
- ⚡ **SignalR 即時通訊**：聊天室、PvP 程式碼對戰、線上課堂
- 🔐 **完整認證系統**：Email + BCrypt、Google OAuth、訪客模式、忘記密碼
- 📊 **背景服務**：ErrorScannerService 自動掃描錯誤、產生日誌
- 🧠 **AI 整合**：Claude Code 協作開發全紀錄，每個 commit 都可追溯

---

### 🔹 [SideProjects — ASP.NET Core MVC 練習](https://github.com/a0936480350/SideProjects)

> ASP.NET Core MVC 基礎練習專案，涵蓋 MVC 架構、CRUD、資料流。

**技術**：C# · ASP.NET Core MVC · Razor · SQL

---

### 🔹 [practice-work — 綜合練習](https://github.com/a0936480350/practice-work)

> 日常開發練習與小型專案集合。

---

## 🎓 技術深度

### 後端
- **C#** — OOP、LINQ、async/await、泛型、委派/事件
- **ASP.NET Core** — MVC、Web API、Minimal API、SignalR、Middleware、DI 容器
- **EF Core** — Code First、Migration、效能優化、N+1 處理
- **SQL** — Query 優化、Index 設計、Stored Procedure、Window Functions

### 前端（持續學習）
- **JavaScript** — ES6+、Promise、DOM、Fetch API
- **HTML/CSS** — Flexbox、Grid、RWD、動畫效果
- **遊戲開發** — Phaser.js 3（2D 遊戲引擎）
- **即時通訊** — SignalR Client（WebSocket）

### 架構思維
- **SOLID 原則** — 單一職責、開閉、里氏替換、介面隔離、依賴反轉
- **設計模式** — Repository、Strategy、Factory、Observer、Decorator
- **系統設計** — 認證機制（Session/JWT/OAuth）、快取策略
- **測試策略** — Mock vs 真實依賴

### DevOps
- **Azure** — App Service、PostgreSQL Flexible Server、部署自動化
- **Git** — 分支策略、Rebase vs Merge、團隊協作
- **CI/CD** — GitHub Actions 基礎

---

## 🛠️ 工具與環境

| 類別 | 使用中 |
|------|--------|
| IDE | Visual Studio 2022、VS Code |
| 資料庫 | PostgreSQL、SQL Server |
| 版控 | Git、GitHub |
| 部署 | Azure App Service |
| AI 輔助 | Claude Code（重度使用） |

---

## 📖 學習筆記

透過 DevLearn 平台整理了自己的學習筆記，包含：
- **語法基礎篇**（200+ 章）— C#、SQL、JavaScript、HTML/CSS、Vue、React
- **概念深入篇**（28 章）— DI 為什麼不直接 new？async 的真相、REST 哲學、系統設計

> 歡迎到 [DevLearn 網站](https://devlearn-dotnet.azurewebsites.net/) 看完整內容！

---

## 📬 聯絡方式

- 💼 GitHub：[@a0936480350](https://github.com/a0936480350)
- 🌐 線上作品：[DevLearn 學習平台](https://devlearn-dotnet.azurewebsites.net/)
- 📱 LINE 官方帳號（DevLearn）：[lin.ee/68vD9ZW](https://lin.ee/68vD9ZW)

---

<sub>⭐ 如果你覺得 DevLearn 有幫助，歡迎在 GitHub 給個 Star！</sub>
